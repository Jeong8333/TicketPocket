...
csv_filename = 'dance.csv'      # 크롤링 공연 정보 저장 파일명
...
url = 'https://tickets.interpark.com/contents/genre/classic'    # 인터파크 티켓 - 발레/무용 페이지 URL
# 백그라운드 실행
...
# '발레/무용' 탭 클릭
driver.find_element(By.XPATH, '//*[@id="contents"]/article[1]/section/div/div/div/button[5]').click()
time.sleep(1)
    ...
    while True:
        ...
        for a in a_arr:
            ...
            if performance_info not in scraped_info:
                ...
                mydb.insert(sql, ["DN02", title, poster, period_date, loc])   #DN02 발레로 저장
