...
csv_filename = 'exhibition.csv'      # 크롤링 공연 정보 저장 파일명
...
url = 'https://tickets.interpark.com/contents/genre/exhibition?app_header_state=hide'    # 인터파크 티켓 - 전시회 페이지 URL
# 백그라운드 실행
...
# '전시회' 탭 클릭
driver.find_element(By.XPATH, '//*[@id="contents"]/article[1]/section/div/div/div/button[3]').click()
time.sleep(1)
    ...
    while True:
        ...
        for a in a_arr:
            ...
            if performance_info not in scraped_info:
                ...
                mydb.insert(sql, ["EX00", title, poster, period_date, loc])
